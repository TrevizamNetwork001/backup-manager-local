...
def dashboard_page_content(
    *,
    ...
    timeline_link = '<a class="panel-link" href="/reports/audit">Ver todos os eventos</a>' if user["can_admin"] else ""
    ...
    def attention_url(item):
        if item["source"] == "Telegram backup":
            view = "history" if "falha" in item["message"] else "queue"
            return f"/telegram-backup?view={view}"
        return "/jobs"

    attention_details_url = attention_url(snapshot["alerts"][0]) if snapshot["alerts"] else "/jobs"
    ...
    attention_lines = "".join(
        f'<li><i>◆</i><span><a href="{e(attention_url(item))}"><strong>{e(term_names.get(item["source"], item["source"]))}</strong> — {e(str(item["message"]).replace("worker", "processamento"))}</a></span></li>'
    ...
    content = render_template(
        ...
        attention_details_url=e(attention_details_url),
